...
//----------------------------------------------
Shader "Kirnu/Marvelous/CustomLightingSoftFogGradient" {
	...
	SubShader {
		...
		Tags { "LIGHTMODE"="ForwardBase" "QUEUE"="Geometry" "RenderType"="Opaque" }
			...
			CGPROGRAM
				...
				#ifndef USE_DIST_FOG
					fixed4 c = customLightingSoftFogFrag(v, _FogColor, _FogHeight, _LightTint, _UseLightMap, _LightmapPower, _LightmapColor, _ShadowPower, 1);
				
					return c;
				#else
					fixed4 c = customLightingSoftFogFrag(v, _FogColor, _FogHeight, _LightTint, _UseLightMap, _LightmapPower, _LightmapColor, _ShadowPower, 1);
					fixed4 d = lerp(half4(_FogColor,1),c,v.color.w);
					
					//half fogDensity = clamp((v.wpos.y - _FogHeight),0,1);
					half fogDensity = (v.wpos.y - _FogHeight) / (_FogHeight) * (1 - step(v.wpos.y, _FogHeight));
					d.xyz += lerp(d.xyz, _TopColor, fogDensity );
					return d;
				#endif
				}
