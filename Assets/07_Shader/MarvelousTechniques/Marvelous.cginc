...
fixed4 customLightingFrag(CL_OUT_WPOS v, half3 lightTint, half useLightmap, half lightmapPower, half3 lightmapColor,half _ShadowPower, half colorBoost = 1){
...
#ifdef USE_FOG
fixed4 customLightingHardFogFrag(CL_OUT_WPOS v,float fogYStartPos, half3 fogColor, half3 lightTint, half useLightmap, half lightmapPower, half3 lightmapColor,half _ShadowPower ,half ColorBoost = 1){

	fixed4 outColor = customLightingFrag(v, lightTint, useLightmap, lightmapPower, lightmapColor, _ShadowPower, ColorBoost);
	return  fixed4(lerp( fogColor, outColor.xyz, v.wpos.y > fogYStartPos),0);
}

fixed4 customLightingSoftFogFrag(CL_OUT_WPOS v, half3 fogColor, half fogHeight, half3 lightTint, half useLightmap, half lightmapPower, half3 lightmapColor,half _ShadowPower , half ColorBoost = 1){
	fixed4 outColor = customLightingFrag(v, lightTint, useLightmap, lightmapPower, lightmapColor, _ShadowPower, ColorBoost);
