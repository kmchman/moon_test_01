...
//----------------------------------------------
Shader "Kirnu/Marvelous/CustomLightingSoftFogColorFlashing" {
	...
	Properties {
		...
		[Toggle(USE_FLASH)]_UseFlash ("Color Flash" ,Float) = 0
		_FlashColor ("Flash Color", Color) = (1,0.73,0.117,0)
	...
	SubShader {
		...
		Tags { "LIGHTMODE"="ForwardBase" "QUEUE"="Geometry" "RenderType"="Opaque" }
			...
			CGPROGRAM
				...
				#pragma shader_feature USE_FLASH
				...
				uniform half4 _FlashColor;
				uniform half _FlashAmount;
				...
				fixed4 frag(CL_OUT_WPOS v) : COLOR {
				...
				#ifndef USE_DIST_FOG
					fixed4 d = customLightingSoftFogFrag(v, _FogColor, _FogHeight, _LightTint, _UseLightMap, _LightmapPower, _LightmapColor, _ShadowPower);
					#if USE_FLASH
					
						d.rgb = lerp(d.rgb,_FlashColor.rgb,_FlashAmount);
						d.rgb *= d.a;
					#endif
					return d;
				#else
					...
					fixed4 e = lerp(half4(_FogColor,1),c,v.color.w);
					#if USE_FLASH
						e.rgb = lerp(e.rgb,_FlashColor.rgb,_FlashAmount);
						e.rgb *= e.a;
					#endif
					return e;
				#endif
